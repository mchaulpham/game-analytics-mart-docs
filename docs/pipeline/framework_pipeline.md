...
* mỗi nhóm bảng phục vụ mục đích phân tích nào;
* các bảng và view phụ thuộc lẫn nhau ra sao;
* nên đọc và sử dụng các bảng theo thứ tự nào.

Tài liệu này là tài liệu gốc để đọc trước khi đi vào từng bản đặc tả chi tiết của 20 bảng và view trong hệ thống.
...
| Event      | Sự kiện được gửi từ game lên GA4. Ví dụ: `Start_level`, `End_level`, `Tutorial_start`.                                                                                |
| Funnel     | Phễu chuyển đổi. Đây là chuỗi bước mà người chơi cần đi qua. Ví dụ: `first_open` → `Open_first` → bắt đầu Level 1.                                                    |
| Cohort     | Nhóm người dùng theo cùng điều kiện bắt đầu phân tích. Ví dụ: nhóm người dùng đã mở game lần đầu và đã đủ 24 giờ để quan sát hành vi.                                 |
| Grain      | Độ chi tiết của mỗi dòng dữ liệu. Ví dụ: một dòng là một lần chơi level, một người chơi theo một level, hoặc một ngày theo một level.                                 |
...
Hệ thống hiện có 20 đối tượng dữ liệu:

| Nhóm                        | Số lượng | Loại đối tượng         |
| --------------------------- | -------: | ---------------------- |
| Nền tảng lõi                |        8 | 2 bảng vật lý, 6 view  |
| Phân tích hiện tại          |        7 | 7 view                 |
| Giám sát và phân tích level |        5 | 5 view                 |
| Tổng cộng                   |       20 | 2 bảng vật lý, 18 view |
...
Luồng dữ liệu có bốn tầng chính:
...
3. **Tầng phân tích hiện tại**
   Tầng này phục vụ các phân tích đang cần dùng trực tiếp cho dự án: phễu người chơi mới, 10 level đầu, thời gian đi qua level, và chẩn đoán thiết kế level.
...
Các view này là điểm vào chính từ dữ liệu thô sang bộ dữ liệu phân tích.
...
Tầng này gồm 7 view, phục vụ trực tiếp cho các câu hỏi phân tích đang cần trong giai đoạn hiện tại của dự án.
...
## 11. Những nguyên tắc phân tích chính

### 11.1 Không phân tích độ khó từ dữ liệu bị ảnh hưởng bởi hướng dẫn nếu không tách riêng
...
### 11.2 Phải kiểm tra tỷ lệ ghép được `End_level`
...
### 11.3 Cần dùng nhóm đã đủ thời gian quan sát

Các view có hậu tố `_24h` dùng nhóm người chơi đã đủ 24 giờ kể từ mốc bắt đầu phân tích.

Mục đích là tránh kết luận sai khi người chơi chưa có đủ thời gian đi tiếp.

### 11.4 Cần tách rơi trước khi thắng và rơi sau khi thắng
...
## 12. Cấu trúc thư mục đề xuất trên GitHub
...
## 13. Cấu trúc chuẩn cho mỗi bản đặc tả chi tiết
...
## 1. Mục đích
## 2. Câu hỏi phân tích có thể trả lời
## 3. Độ chi tiết của mỗi dòng dữ liệu
## 4. Nguồn dữ liệu phụ thuộc
## 5. Danh sách cột đầu ra
## 6. Trường khóa và trường định danh quan trọng
## 7. Chỉ số chính
## 8. Logic xử lý dữ liệu
## 9. Bộ lọc và điều kiện chọn dữ liệu
## 10. Lưu ý về chất lượng dữ liệu
...
* “chỉ số chính” giải thích các cột đo lường như tỷ lệ thắng, tỷ lệ rơi, số người chơi;
...
## 14. Thứ tự đọc khuyến nghị cho người mới trong team
...
3. Nhóm phân tích người chơi mới và 10 level đầu:
   - vw_onboard_funnel_24h.md
   - vw_onboard_drop_diag_24h.md
   - vw_f10_funnel_24h.md
   - vw_f10_timing_24h.md
...
4. Nhóm chẩn đoán thiết kế level:
   - vw_f10_design_diag_24h.md
   - vw_f10_design_board_24h.md
...
## 15. Trạng thái hiện tại của kiến trúc

Tính đến thời điểm tài liệu này được viết:

```text
Tổng số đối tượng: 20
Bảng vật lý: 2
View: 18
